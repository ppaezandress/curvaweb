    ...
    <div className="relative border-t border-line pt-3">
      ...
      {trigger && matches.length > 0 && (
        <div className="absolute bottom-full left-0 mb-2 max-h-64 w-full max-w-md overflow-y-auto rounded-2xl border border-line bg-white shadow-float">
          ...
          {matches.map((it) =>
            trigger.kind === "user" ? (
              <button key={(it as Member).id} onClick={() => pickPerson(it as Member)} className="flex w-full items-center gap-2 px-3 py-2 text-left transition hover:bg-zinc-50 focus-ring">
                <Avatar member={it as Member} size={24} />
                <span className="truncate text-sm text-ink">{(it as Member).name}</span>
              </button>
            ) : (
              <button key={(it as Task).id} onClick={() => pickTask(it as Task)} className="flex w-full items-center gap-2 px-3 py-2 text-left transition hover:bg-zinc-50 focus-ring">
                <ListTodo size={14} className="shrink-0 text-curva-purple" />
                <span className="truncate text-sm text-ink">{(it as Task).name}</span>
              </button>
            ),
          )}
        </div>
        ...
        <div className="mb-2 flex flex-wrap gap-1.5">
          ...
          {pendingTasks.map((t) => (
            <span key={t.id} className="inline-flex items-center gap-1 rounded-full bg-curva-purple/10 py-0.5 pl-2 pr-1 text-xs font-medium text-curva-purple">
              ...
              <button onClick={() => setPendingTasks((p) => p.filter((x) => x.id !== t.id))} className="rounded-full p-0.5 hover:bg-curva-purple/20 focus-ring" aria-label="Quitar"><X size={11} /></button>
            </span>
          ))}
        </div>
      )}
